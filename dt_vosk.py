...
file_dir = file_path.replace('dt.py','')
...
# ensure that all tiers end at the same time
# tg_keys = sorted_textgrid.keys()
# for k in tg_keys:
#     if sorted_textgrid[k][1][-1][3] < tg_max_time:
#         if sorted_textgrid[k][1][-1][0] == '':
#             sorted_textgrid[k][1][-1][3] = tg_max_time
#         else:
#             sorted_textgrid[k][1].append(['',0,sorted_textgrid[k][1][-1][3],tg_max_time])
...
with open('dt_log.txt', 'a') as f:
    f.write('\t'.join([started_diarization.strftime("%Y-%m-%d %H:%M:%S"), wav_path, model_name, tg_path, str(duration), str(len(word_counts)), str(round(diarization_time.total_seconds())), str(round(transcription_time.total_seconds()))])+'\n')
